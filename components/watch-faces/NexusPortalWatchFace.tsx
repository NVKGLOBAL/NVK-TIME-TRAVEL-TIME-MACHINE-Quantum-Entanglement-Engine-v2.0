...
const NexusPortalWatchFace: React.FC<WatchFaceProps> = ({ time, modeName = "Nexus Portal" }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            ctx.lineWidth = 2;
            for (let i = 0; i < 12; i++) {
                const angle = (i / 12) * Math.PI * 2 + timestamp * 0.001;
                const r = radius * (0.4 + Math.sin(timestamp * 0.002 + i) * 0.1);
                ctx.beginPath();
                ctx.moveTo(centerX, centerY);
                ctx.quadraticCurveTo(centerX + Math.cos(angle + 0.5) * r * 1.5, centerY + Math.sin(angle + 0.5) * r * 1.5, centerX + Math.cos(angle) * r, centerY + Math.sin(angle) * r);
                ctx.stroke();
            }
...
export default NexusPortalWatchFace;
