...
const RealityFragmentWatchFace: React.FC<WatchFaceProps> = ({ time, modeName = "Reality Fragment" }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            ctx.lineWidth = 1;
            for (let i = 0; i < 8; i++) {
                const angle = (i / 8) * Math.PI * 2 + timestamp * 0.0002;
                const r = radius * 0.8;
                ctx.beginPath();
                ctx.moveTo(centerX, centerY);
                ctx.lineTo(centerX + Math.cos(angle) * r, centerY + Math.sin(angle) * r);
                ctx.lineTo(centerX + Math.cos(angle + 0.5) * r * 0.7, centerY + Math.sin(angle + 0.5) * r * 0.7);
                ctx.closePath();
                ctx.stroke();
            }
...
export default RealityFragmentWatchFace;
