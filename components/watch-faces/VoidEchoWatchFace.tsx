...
interface VoidEchoWatchFaceProps {
    time: Date;
    modeName: string;
}

const VoidEchoWatchFace: React.FC<VoidEchoWatchFaceProps> = ({ time, modeName }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            // Void Echo (Expanding Rings)
            ctx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
            ctx.lineWidth = 1;
            const numRings = 5;
            for (let i = 0; i < numRings; i++) {
                const ringRadius = (radius * (timestamp * 0.0002 + i / numRings)) % radius;
                const opacity = 1 - ringRadius / radius;
...
export default VoidEchoWatchFace;
