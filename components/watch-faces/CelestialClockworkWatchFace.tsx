...
const CelestialClockworkWatchFace: React.FC<WatchFaceProps> = ({ time, modeName = "Celestial Clockwork" }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                const angle = timestamp * 0.0005 * (i + 1);
                ctx.beginPath();
                ...
                ctx.beginPath();
                ctx.arc(centerX + Math.cos(angle) * r, centerY + Math.sin(angle) * r, 3, 0, Math.PI * 2);
                ctx.fill();
            }
...
export default CelestialClockworkWatchFace;
