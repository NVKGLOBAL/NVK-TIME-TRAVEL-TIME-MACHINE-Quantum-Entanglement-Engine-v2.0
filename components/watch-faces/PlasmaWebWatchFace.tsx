...
interface PlasmaWebWatchFaceProps {
    time: Date;
    modeName: string;
}

const PlasmaWebWatchFace: React.FC<PlasmaWebWatchFaceProps> = ({ time, modeName }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            for (let i = 0; i < numLines; i++) {
                ...
                const lineHue = (timestamp * 0.1 + i * 12) % 360;
                ctx.strokeStyle = `hsla(${lineHue}, 70%, 60%, 0.2)`;
                ctx.stroke();
            }
...
export default PlasmaWebWatchFace;
