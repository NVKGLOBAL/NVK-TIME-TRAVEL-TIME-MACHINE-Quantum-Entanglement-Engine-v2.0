...
interface NullShellWatchFaceProps {
    time: Date;
    modeName: string;
}

const NullShellWatchFace: React.FC<NullShellWatchFaceProps> = ({ time, modeName }) => {
    ...
    useEffect(() => {
        ...
        const render = (timestamp: number) => {
            ...
            for (let i = 0; i < numShells; i++) {
                const shellRadius = radius * (0.3 + (i / numShells) * 0.7);
                const angleOffset = timestamp * 0.0001 * (i % 2 === 0 ? 1 : -1);
                
                ctx.beginPath();
                ctx.arc(centerX, centerY, shellRadius, angleOffset, angleOffset + Math.PI * 1.5);
                ctx.stroke();
            }
...
export default NullShellWatchFace;
